...
typedef int (*ecat_slave_init_t) (ecx_contextt *context, uint16 slave);//(int comp_id, struct ec_slavet_t *slave, ec_pdo_entry_reg_t *pdo_entry_regs);
typedef void (*ecat_slave_rw_t) (ecx_contextt *context, uint16 slave);	
...
int ec_findconfig( uint32 man, uint32 id)
{
   ...
   do 
   {
	  ...
	  {
	    EC_PRINT("slave %d 's configer man=0x%x, configer id=0x%x\n",i, ec_configlist[i].man, ec_configlist[i].id);
	    EC_PRINT("slave %d 's true man=0x%x, true id=0x%x\n", i,man, id);
	  }
	...
	if (wkc > 0)
   {
      ...
      else
      {
         EC_PRINT("Error: too many slaves on network: num_slaves=%d, EC_MAXSLAVE=%d\n",
               wkc, EC_MAXSLAVE);
         return -4;/** return value too many slaves */
      }
   }
   return wkc;
}
...
//   b = 0x00;
//   ecx_BWR(context->port, 0x0000, ECT_REG_DCSYNCACT   , sizeof(b) , &b   , EC_TIMEOUTRET3);  /* disable DC */
   ...
   ecx_BWR(context->port, 0x0000, ECT_REG_ALCTL       , sizeof(b) , &b, EC_TIMEOUTRET3);     /* Reset all slaves to Init */
   b = 2;
   ecx_BWR(context->port, 0x0000, ECT_REG_EEPCFG      , sizeof(b) , &b, EC_TIMEOUTRET3);     /* force ECAT access to Eeprom from PDI */
   b = 0;
...
static int ecx_config_from_table(ecx_contextt *context, uint16 slave)
{
   ...
   if (cindex)
   {
	  ...
	  EC_PRINT("slave %d 's SM2a=0x%x\n", slave,csl->SM[2].StartAddr);
	  ...
	  EC_PRINT("slave %d 's SM3a=0x%x\n", slave,csl->SM[3].StartAddr);
...
int ecx_config_init(ecx_contextt *context, uint8 usetable)
{
   ...
   ecx_init_context(context);       /* 初始化变量 */
	wkc = ecx_detect_slaves(context);/* 查找从站 wkc>0 查找到从站，反之则未找到从站 */
	 if(wkc>0)
	 {
         ecx_set_slaves_to_default(context); /* 初始化从站 */
		     // 设置 从站 node id
				 for (slave = 1; slave <= *(context->slavecount); slave++)
						{
							 ...
							 configadr = etohs(ecx_APRDw(context->port, ADPh, ECT_REG_STADR, EC_TIMEOUTRET3));  // get configure address						 
							 context->slavelist[slave].configadr = configadr;

							 ecx_FPRD(context->port, configadr, ECT_REG_ALIAS, sizeof(aliasadr), &aliasadr, EC_TIMEOUTRET3);  // get  alias address				   
						...
						for (slave = 1; slave <= *(context->slavecount); slave++)
						{
							 ...
							 context->slavelist[slave].mbx_wo = (uint16)LO_WORD(eedat); /* write mailbox offset */
							 context->slavelist[slave].mbx_l = (uint16)HI_WORD(eedat);  /* write mailbox length */
...
//									ecx_readeeprom1(context, slave, ECT_SII_MBXPROTO);
							 ...
							 /* ptype = Physical type*/
							 context->slavelist[slave].ptype = LO_BYTE(etohs(ecx_FPRDw(context->port, configadr, ECT_REG_PORTDES, EC_TIMEOUTRET3)));
							 ...
							 {   
									...
									context->slavelist[slave].mbx_proto = ecx_readeeprom2(context, slave, EC_TIMEOUTEEP);
							 }   
							 cindex = 0;
							 ...
							 if (context->slavelist[slave].mbx_l > 0)
							 {
									...
									ecx_FPWR(context->port, configadr, ECT_REG_SM0, sizeof(ec_smt) * 2, &(context->slavelist[slave].SM[0]), EC_TIMEOUTRET3);
							 } 
...
//						   ecx_statecheck(context, slave, EC_STATE_PRE_OP, EC_TIMEOUTSTATE);
						}	
		 }
   return wkc;
}
...
//	 if(wkc > 0)
   {
      ...
      b = 0xf0;
      ecx_APWRw(context->port, ADPh, ECT_REG_DLPORT , b, EC_TIMEOUTRET3);     /* deact loop manual */
      zbuf=0x00000000;
		  ecx_APWRw(context->port, ADPh, ECT_REG_RXERR  , zbuf, EC_TIMEOUTRET3);  /* reset CRC counters */		 
		  ecx_APWRw(context->port, ADPh, ECT_REG_ALCTL, htoes(EC_STATE_INIT|EC_STATE_ACK) , EC_TIMEOUTRET3);
...
/*
 * ecat_loop() — DEPRECATED for this project.
 * Old single-axis state machine removed.
 * 4-axis control is now in Core/Src/main.c (Axis_EnableAll + Axis_ControlCycle).
 * TIM1 callback (HAL_TIM_PeriodElapsedCallback) directly calls
 * ec_send_processdata() and sets pdoTimeFlag for the main loop.
 */
#endif
