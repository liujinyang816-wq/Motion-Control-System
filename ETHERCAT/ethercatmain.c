...
{
   return ecx_setupnic(context->port, FALSE);
}
...
int ecx_init_redundant(ecx_contextt *context, ecx_redportt *redport)
{
   ...
   ecx_setupnic(context->port, FALSE);
   rval = ecx_setupnic(context->port, TRUE);
...
uint16 ecx_statecheck(ecx_contextt *context, uint16 slave, uint16 reqstate, int timeout)
{
   ...
   while ((state != reqstate) && ((bsp_GetTickCount()-timer2)<timeout));
...
int ecx_mbxempty(ecx_contextt *context, uint16 slave, int timeout)
{
   ...
   do
   {
      ...
      {
         Delay_us(EC_LOCALDELAY);
      }
   }
   while (((wkc <= 0) || ((SMstat & 0x08) != 0)) && ( (bsp_GetTickCount()-timer2)<timeout));
            ...
            else
            {
               if (wkc <= 0) /* read mailbox lost */
               {
                  ...
                  wkc2 = ecx_FPWR(context->port, configadr, ECT_REG_SM1STAT, sizeof(SMstat), &SMstat, EC_TIMEOUTRET);
								 SMstat = etohs(SMstat);
				        timer2[1]=bsp_GetTickCount();
                  do /* wait for toggle ack */
                  {
                     wkc2 = ecx_FPRD(context->port, configadr, ECT_REG_SM1CONTR, sizeof(SMcontr), &SMcontr, EC_TIMEOUTRET);
									} while (((wkc2 <= 0) || ((SMcontr & 0x02) != (HI_BYTE(SMstat) & 0x02))) && (bsp_GetTickCount()-timer2[1]<timeout));
             timer2[2]=bsp_GetTickCount();
				   do /* wait for read mailbox available */
                  {
                  ...
                  } while (((wkc2 <= 0) || ((SMstat & 0x08) == 0)) && (bsp_GetTickCount()-timer2[2]<timeout));
               }
            }
         } while ((wkc <= 0) && (bsp_GetTickCount()-timer2[3]<timeout)); /* if WKC<=0 repeat */
      }
...
uint16 ecx_eeprom_waitnotbusyAP(ecx_contextt *context, uint16 aiadr,uint16 *estat, int timeout)
{
   ...
   while (((wkc <= 0) || ((*estat & EC_ESTAT_BUSY) > 0)) && ((bsp_GetTickCount()-timer2)<timeout)); /* wait for eeprom ready */
...
int ec_init(void)  //更改；不需要传参数
{
	 if(socket_mode==1)
      ecx_init(&ecx_context);
	 else if(socket_mode==2)
     ecx_init_redundant(&ecx_context,&ecx_redport);
	 return 0;
}
