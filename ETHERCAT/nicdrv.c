...
uint8_t DM9000_LINK_OK;
uint8_t DM9161_LINK_OK;
...
int ecx_outframe(ecx_portt *port, int idx, int stacknumber)
{
   ...
   if (stacknumber==0)
   {
	  ...
	  bfin_EMAC_send ((*stack->txbuf)[idx], lp);
   }
   else
   {
...
//   stack = &(port->stack);
		...
		if (stacknumber==0)
      {
           ...
           if(bfin_EMAC_recv ((uint8*)receiveBuffer, lp))
			{ 
...
//				    printf("DM9000 Rx_Buff is:");
//					for(j=0;j<16;j++)
//					  printf("%x ", receiveBuffer_DM9000[j]);
//					printf("\n\r");	
           }
      }	 	
		 rval = EC_NOFRAME;		 
